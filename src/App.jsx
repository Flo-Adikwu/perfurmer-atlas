...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen md:flex-row bg-background text-charcoal font-inter overflow-hidden">
      {/* Left Panel: Note Browser */}
      <div className="w-full md:w-1/3 lg:w-1/4 border-r border-moss/20 flex flex-col h-full bg-white shadow-sm z-10">
        <header className="p-6 border-b border-moss/10 bg-background/50">
            ...
            <div 
              ...
              className={`p-4 rounded-xl cursor-pointer transition-all border group relative ${
                (compareMode ? compareList.includes(note.id) : selectedNoteId === note.id)
                  ? 'bg-moss text-white border-moss shadow-lg scale-[1.02]' 
                  : 'bg-white border-moss/10 hover:border-moss/30 hover:shadow-md'
              }`}
            >
              ...
              {!compareMode && (
                <button 
                  ...
                  className={`absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity ${
                    favourites.includes(note.id) ? 'text-terracotta opacity-100' : 'text-moss/30 hover:text-moss/60'
                  }`}
                >
        ...
        <footer className="p-4 border-t border-moss/10 bg-white">
          <button 
            onClick={() => {
          ...
          {compareMode && (
            <p className="text-[10px] text-center mt-2 text-charcoal/50 uppercase tracking-widest font-bold">Select up to 3 notes</p>
          )}
        </footer>
      </div>

      {/* Right Panel: Note Detail or Comparison */}
      <div className="flex-1 overflow-y-auto bg-background relative overflow-hidden">
        {compareMode ? (
          ...
          <div className="h-full">
            ...
            {selectedNote ? (
              <div className="max-w-4xl mx-auto p-8 lg:p-16 relative z-10">
                <div className="mb-12">
                      ...
                      {selectedNote.pairsWith.map(pairId => {
                        ...
                        return (
                          <button 
                            ...
                            className="px-5 py-2.5 bg-moss text-white rounded-full text-sm font-medium hover:bg-moss/90 transition-all hover:shadow-md hover:-translate-y-0.5 active:translate-y-0"
                          >
                            {pair?.name || pairId}
                          </button>
                        )
                      ...
                      {selectedNote.clashesWith.map(clashId => {
                        ...
                        return (
                          <button 
                            ...
                            className="px-5 py-2.5 bg-terracotta text-white rounded-full text-sm font-medium hover:bg-terracotta/90 transition-all hover:shadow-md hover:-translate-y-0.5 active:translate-y-0"
                          >
                            {clash?.name || clashId}
                          </button>
                        )
